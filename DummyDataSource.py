...
class DummyDataSource (Ds.DataSource):
   ...
   def setRange(self, minimum, maximum):
      self.max = maximum
      self.min = minimum
      # print("Setting", self.getName(), "max to", maximum, "and min to", minimum)
   ...
   def getValue(self):
      if self.getName() != "Pump":
         ...
         diff = self.offset + (diff * self.period / 360) # and the randomisers
         diff = math.radians(diff)                     # rotate approx every 6 minutes
