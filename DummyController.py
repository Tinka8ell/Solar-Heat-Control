...
class Controller (Tk.Frame):
   ...
   def __init__(self, master):
      ...
      self.logger = Tk.Text(logframe, yscrollcommand = yscrollbar.set, xscrollcommand = xscrollbar.set, width = 40, height = 20, wrap = Tk.NONE)
      ...
      if changeHappened:
         ...
         if was != self.values["Pump"]:
            if self.values["Photo"]  == 0:
               self.log("Turning pump on as photo now " + self.values["Photo"])
            else:
               self.log("Turning pump off as photo is " + self.values["Photo"])
...
Flow  = 999.9  Power = 999.9
      ...
      text = ["Time" + chr(9) + self.currentTime + chr(9) + "Next" + chr(9) + self.nextTime,
              "Period:" + chr(9) + str(self.getPeriod()) + chr(9) + "Threshold" + chr(9) + str(self.properties["Threshold"]),
              "Light:" + chr(9) + str(self.values.get("Photo")) + chr(9) + "Pump:" + chr(9) + str(self.values.get("Pump")),
              "Pool:" + chr(9) + str(self.values.get("Pool")) + DEG_C + chr(9) + "Water:" + chr(9) + str(self.values.get("Water")) + DEG_C,
   ...
   def log(self, message):
      self.logger.insert(Tk.END, self.clock["text"] + ": " + message + chr(10))
